...
with hist as (
    select
        upper(trim(geographic_id)) as geographic_id,
        trim(dataset_id) as dataset_id,
        trim(source_dataset_id) as source_dataset_id,
        upper(trim(dataset_product_id)) as dataset_product_id,
        ...
        upper(trim(competitor_indicator)) as competitor_indicator,
        upper(trim(generic_indicator)) as generic_indicator,
        upper(trim(prescription_required_indicator)) as prescription_required_indicator,
    ...
    from {{ var('closeup_hist_database', target.database) }}.{{ var('closeup_hist_schema', 'DW_DFHPMS2EU_SEMARCHY_SCHEMA') }}.{{ var('closeup_product_hist_table', 'CLOSEUP_PRODUCT_HIST') }}
...
with_md5 as (
    select
        hist.*,
        md5(concat_ws('||',
            coalesce(geographic_id,''), coalesce(dataset_id,''),
            coalesce(source_dataset_id,''), coalesce(dataset_product_id,''))) as hashkey,
        md5(concat_ws('||',
            coalesce(geographic_id,''), coalesce(dataset_id,''),
            coalesce(source_dataset_id,''), coalesce(product_type_code,''),
            coalesce(dataset_product_id,''), coalesce(product_name,''),
            coalesce(standardized_product_name,''), coalesce(description,''),
            coalesce(product_status_code,''), coalesce(generic_names,''),
            coalesce(cast(effective_date as varchar),''), coalesce(cast(end_date as varchar),''),
            coalesce(brand_name,''), coalesce(cast(approved_date as varchar),''),
            ...
            coalesce(substance,''), coalesce(dose_form_code,''), coalesce(strength,''),
            coalesce(route_of_administration_code,''),
            coalesce(pharmaceutical_product_quantity,''),
            coalesce(unit_of_presentation_code,''), coalesce(primary_package_quantity,''),
            coalesce(primary_package_type_code,''), coalesce(secondary_package_quantity,''),
            coalesce(secondary_package_type_code,''), coalesce(package_quantity,''),
            coalesce(shelf_life_quantity,''),
            coalesce(shelf_life_unit_of_measure_code,''), coalesce(sample_indicator,''),
            coalesce(competitor_indicator,''), coalesce(generic_indicator,''),
            coalesce(prescription_required_indicator,''),
            coalesce(atc_code,''),
            coalesce(who_code,''), coalesce(nfc_code,''), coalesce(chc_nec_code,''),
            coalesce(usc_code,''), coalesce(manufacturer_id,''),
            coalesce(daily_dosage_quantity,''), coalesce(corporation,''),
            coalesce(manufacturer,''), coalesce(is_imported,''),
            coalesce(generic_product_description,''), coalesce(channel,''),
            coalesce(chc_flag,''))) as hashdiff
    from hist
),
