{#
    MACRO: delta_load_identifier_merge
    ============================================================
    Purpose:
        MERGE-based delta/CDC load for Veeva CRM product identifier data
    ...
    Delta Logic Overview:
        ...
        2. CHANGED records (hashkey match BUT hashdiff mismatch):
           -> Archive the old SAT record to SAT_IDENTIFIER_HIST with END_DATETIME.
        ...
        3. NEW records (no hashkey match in HUB):
           ...
           -> Insert into SAT_IDENTIFIER with START_DATETIME.
    ...
    Tables Involved:
        - Source: stg_veeva_crm_product_identifier (dbt view)
        - Hub:    DEV_DFHPMS2EU_DB.DW_DFHPMS2EU_SEMARCHY_SCHEMA.HUB
        - Sat:    DEV_DFHPMS2EU_DB.DW_DFHPMS2EU_SEMARCHY_SCHEMA.SAT_IDENTIFIER
        - Hist:   DEV_DFHPMS2EU_DB.DW_DFHPMS2EU_SEMARCHY_SCHEMA.SAT_IDENTIFIER_HIST

    Usage:
        dbt run-operation delta_load_identifier_merge
#}
{% macro delta_load_identifier_merge(
    stg_model='STG_IDENTIFIER_VEEVA',
    hub_model='HUB',
    sat_model='SAT_IDENTIFIER',
    hist_model='SAT_IDENTIFIER_HIST',
    reject_model='REJECT',
    ...
    {% set tmp_dq_table = sat_table.database ~ '.' ~ sat_table.schema ~ '.TMP_STG_IDENTIFIER_DQ' %}
    {% set tmp_delta_table = sat_table.database ~ '.' ~ sat_table.schema ~ '.TMP_STG_IDENTIFIER_DELTA' %}
    {% set tmp_delta_counts_table = sat_table.database ~ '.' ~ sat_table.schema ~ '.TMP_IDENTIFIER_DELTA_COUNTS' %}
    ...
    {% if audit_model_name is none %}
        {% set audit_model_name = 'delta_load_identifier_merge:' ~ stg_model %}
    {% endif %}
    ...
    {% set create_dq_stg_sql %}
        ...
        FROM (
            ...
            WHERE stg.GEOGRAPHIC_ID IS NOT NULL AND TRIM(stg.GEOGRAPHIC_ID) != ''
              ...
              AND stg.IDENTIFIER_ID IS NOT NULL AND TRIM(stg.IDENTIFIER_ID) != ''
    ...
    {% set merge_sat_sql %}
        ...
        WHEN MATCHED AND stg.HASHDIFF != sat.{{ sat_hashdiff_col }} THEN
            UPDATE SET
                ...
                sat.IDENTIFIER_TYPE_CODE = stg.IDENTIFIER_TYPE_CODE,
                sat.IDENTIFIER_ID = stg.IDENTIFIER_ID,
    ...
    {% do log("Step 3: Archiving changed records to SAT_IDENTIFIER_HIST", info=True) %}
